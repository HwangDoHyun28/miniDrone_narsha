...
3. [대회진행 전략](#3-대회진행-전략)
4. [CNN 모델](#4-CNN-모델) 
5. [다항식 곡선 피팅](#5-다항식-곡선-피팅)
6. [각 단계별 알고리즘 정리](#6-각-단계별-알고리즘-정리)
7. [소스코드 설명](#7-소스코드-설명)	
...
> 드론이 앞으로 움직이면 일정 거리부터 구멍의 윗부분이 잘리는 상황이 발생하였다. 모든 1~3m 상황에서 증가되는 구멍의 크기 정도에는 차이가 있더라도, 증가되는 경향은 계속 보였기 때문에,  데이터의 형태가 지수함수나 이차함수 꼴이라고 생각하였다. 각각의 오차율을 토대로 분석한 결과, 이차함수꼴이 더 타당하다는 결과를 얻을 수 있었다.
